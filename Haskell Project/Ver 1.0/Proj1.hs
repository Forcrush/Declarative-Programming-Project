...
computeDistance guess target = max xdis ydis
    where
        xdis = abs (fromEnum (head target) - fromEnum (head guess))
        ydis = abs (fromEnum (last target) - fromEnum (last guess))
...
initialGuess = (guess, initialState)
    where
        ...
        allguess = [[[a,b],[c,d],[e,f]] | a <- ['A'..'H'], c <- ['A'..'H'], e <- ['A'..'H'],
                     b <- ['1'..'4'], d <- ['1'..'4'], f <- ['1'..'4'],
...
nextGuess (lastGuess,currentState) lastScore
    ...
    | otherwise = (nextGuess1,nextState1)
    where
        ...
        nextGuess1 = if currentState /= [] then head currentState else lastGuess
        nextState1 = mydelete nextGuess1 currentState
